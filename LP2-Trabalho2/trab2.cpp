...
Log::Log() {
	cout << "!!!!!!!!! LOG CONSTRUTOR" << endl;
	
	if (freopen("log.txt", "a+", stderr)==NULL) {
		cout << "!!!!!!!!!!!! ERRO CRIANDO LOG" << endl;
	} 
...
int main(int argc, char** argv) {
    ...
    {  
        ...
        int main = accept(sd, 0, 0);
       	inet_ntop(AF_INET, &(client_addr.sin_addr), client_ip, INET_ADDRSTRLEN);  // Funçao que pega o IP do cliente
        string ip(client_ip);
